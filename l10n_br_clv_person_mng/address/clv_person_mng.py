...
class clv_person_mng(models.Model):
    ...
    def addr_zip_search(self, cr, uid, ids, context=None):
        ...
        for clv_person_mng in self.browse(cr, uid, ids):
            addr_zip_ids = obj_addr_zip.zip_search_multi(
                cr, uid, ids, context,
                country_id=clv_person_mng.addr_country_id.id,
                state_id=clv_person_mng.addr_state_id.id,
                l10n_br_city_id=clv_person_mng.addr_l10n_br_city_id.id,
                district=clv_person_mng.addr_district,
                street=clv_person_mng.addr_street,
                zip_code=clv_person_mng.addr_zip,
            )
            ...
            if len(addr_zip_ids) == 1:  # FIXME
                ...
                self.write(cr, uid, [clv_person_mng.id], addr_result, context)
                return True
            else:
